...
import { thunks } from '../store/search'
  ...
  useEffect(() => {
    if (search.results.length > 0) {
      ...
      const scrollHandler = (e) => {
        if ((window.innerHeight + window.scrollY) >= grid.clientHeight) {
          document.removeEventListener('scroll', scrollHandler);
          if(!requested.current){
            requested.current = true
            dispatch(thunks.startSearching())
            setTimeout(() => {
      ...
      <main>
        <Grid container>
          ...
          <Grid item xs={12} style={{
            ...
            {(
              <Grid id='album-grid' container spacing={2} >
